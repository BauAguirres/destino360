...
class CrudReservas {
    ...
    public function vincularReservas($idIda, $idVuelta) {
        $idIda = (int)$idIda;
        $idVuelta = (int)$idVuelta;
        mysqli_query($this->db, "UPDATE reserva SET idReservaRelacionada = $idVuelta WHERE idReserva = $idIda");
        mysqli_query($this->db, "UPDATE reserva SET idReservaRelacionada = $idIda WHERE idReserva = $idVuelta");
    }
    ...
    public function confirmarReserva($idReserva) {
        $idReserva = (int) $idReserva;
        $sql = "UPDATE reserva SET estadoReserva = 'confirmada' WHERE idReserva = $idReserva";
        return mysqli_query($this->db, $sql);
    }

    public function cancelarReserva($idReserva) {
    $idReserva = (int)$idReserva;
    $query = "UPDATE reserva SET estadoReserva = 'cancelada' WHERE idReserva = $idReserva";
    return mysqli_query($this->db, $query);
    }
}